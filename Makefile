...
data-ingestion:
	...
	(cd data_ingestion && uv run data_ingestion_pipeline/submit_pipeline.py \
		...
		--region="us-central1" \
		--vector-search-index="dummy-agent-vector-search" \
		--vector-search-index-endpoint="dummy-agent-vector-search-endpoint" \
		--vector-search-data-bucket-name="$$PROJECT_ID-dummy-agent-vs" \
		--service-account="dummy-agent-rag@$$PROJECT_ID.iam.gserviceaccount.com" \
		--pipeline-root="gs://$$PROJECT_ID-dummy-agent-rag" \
